...
PRODUCT_LIBRARY = [
    ...
    {
        "category": "스마트폰/가전",
        "keywords": ("스마트폰", "휴대폰", "모바일", "가전", "appliance", "tv", "스마트", "갤럭시", "아이폰", "냉장고", "세탁기", "영상", "음향", "audio"),
        "inputs": ["SoC", "이미지센서", "OLED 패널", "파워 IC"],
        "alternatives": ["대만", "베트남", "폴란드", "멕시코"],
        "note": "고가 ODM/EMS와 특수부품 공급이 중요해 중국·일본·미국 수출 규제에 동시 노출됩니다.",
    },
    {
        "category": "배터리/전지",
    ...
    {
        "category": "조선/해양",
        "keywords": ("조선", "선박", "LNG선", "해양플랜트", "ship", "드릴십", "선체", "해양"),
        "inputs": ["후판", "LNG 연료 시스템", "엔진", "스크러버"],
        "alternatives": ["루마니아", "싱가포르", "인도", "사우디"],
        "note": "러시아-우크라 전쟁과 중동 해상 긴장으로 강재·가스 기자재 조달이 지연될 수 있습니다.",
    },
    {
        "category": "바이오/제약",
    ...
    {
        "category": "정밀화학/소재",
        "keywords": ("화학", "폴리머", "석유화학", "정밀화학", "도료", "필름", "소재"),
    ...
    {
        "category": "에너지/정유",
        "keywords": ("정유", "석유", "gas", "lng", "발전", "power plant", "재생에너지", "태양광"),
        "inputs": ["원유", "천연가스", "터빈", "촉매"],
        "alternatives": ["UAE", "카타르", "미국", "노르웨이"],
        "note": "원유 감산, 가스 제재, 기후 규제 변화에 직접 노출됩니다.",
    },
    {
        "category": "통신/IT 서비스",
    ...
    {
        "category": "식품/소비재",
        "keywords": ("식품", "음료", "agro", "음식료", "consumer", "화장품", "뷰티", "생활용품"),
        "inputs": ["곡물", "팜유", "카카오", "향료"],
        "alternatives": ["브라질", "미국", "말레이시아", "베트남"],
        "note": "곡물·원당·팜유 가격과 수출 제한, 위생 규제가 핵심 변수입니다.",
    },
    {
        "category": "물류/상사",
        "keywords": ("물류", "로지스틱스", "상사", "trading", "유통", "shipping", "항만", "포워딩"),
        "inputs": ["선박 슬롯", "창고", "항만 처리", "보험"],
        "alternatives": ["말레이시아", "두바이", "네덜란드", "미국 서안"],
        "note": "항만 적체·컨테이너 운임·보험 제재 등 글로벌 물류 변동이 직격합니다.",
    },
    {
        "category": "철강/소재",
        "keywords": ("철강", "알루미늄", "동", "합금", "파이프", "판재", "제강"),
        "inputs": ["철광석", "제철 석탄", "전력"],
...
COUNTRY_RISK_LIBRARY: Dict[str, dict] = {
    ...
    "브라질": {
        "industries": {"식품/소비재", "에너지/정유", "철강/소재"},
        "risk_tags": ["곡물 수출세", "원유 국영 정책"],
        "alternatives": ["파라과이", "미국", "멕시코"],
    },
    "베트남": {
        "industries": {"스마트폰/가전", "물류/상사", "모빌리티/자동차"},
        "risk_tags": ["전력 부족", "항만 적체"],
        "alternatives": ["태국", "말레이시아"],
    },
}
