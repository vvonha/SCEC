...
Reddit · BBC · Reuters · AP · Al Jazeera · Korea Times · The Guardian · Deutsche Welle · New York Times · VOA · CNN · NPR · Sky News · NHK · South China Morning Post · Straits Times · Al-Monitor 등 다중 뉴스 피드에서 감지한 분쟁 국가쌍과 자동 매칭해 공급망 리스크 리포트를 만들어 주는 로컬 웹 앱입니다. 위키/뉴스 검색과 카테고리별 공급망 디렉터리를 조합해 실제 공급 파트너와 대체 제조사를 실명으로 제시합니다.
...
3. Wikipedia REST 검색을 활용해 "기업명 supplier" / "제품명 manufacturer" 등 질의에서 실명 공급 파트너와 대체 제조사를 추출하고 조달 경로를 계산합니다.
4. Reddit r/worldnews, BBC, Reuters, AP, Al Jazeera, Korea Times, Guardian, DW, NYT, VOA, CNN, NPR, Sky News, NHK, South China Morning Post, Straits Times, Al-Monitor RSS 헤드라인과 지정학 시나리오 라이브러리를 결합해 분쟁 국가쌍을 생성합니다.
5. 품목–분쟁–실명 공급사·대체 제조사 매칭을 통해 위험 포인트, 대체 공급국, 수입선 의존도, 경보 메시지를 계산하고 반응형 UI로 시각화합니다.
...
- `conflict_fetcher.py` – Reddit/BBC/Reuters/AP/Al Jazeera/Korea Times/The Guardian/DW/NYT/VOA/CNN/NPR/Sky/NHK/SCMP/Straits Times/Al-Monitor RSS 헤드라인과 지정학 라이브러리를 통합해 분쟁 국가쌍을 구성합니다.
- `product_source.py` – KIND, Wikipedia 멀티랭 데이터를 결합해 제품/서비스 설명을 수집합니다.
- `supplier_network.py` – Wikipedia REST 검색으로 실명 공급사/대체 제조사를 수집하고 국가·경로·근거 문장을 제공합니다.
- `supply_chain.py` – 품목 카테고리별 대표 수입선·수요처·물류 경로·경보 메시지를 생성합니다.
- `supply_directory.py` – 카테고리별 대표 공급사/대체 제조사/물류 루트를 유지해 제품 카드에 실명 정보를 추가합니다.
